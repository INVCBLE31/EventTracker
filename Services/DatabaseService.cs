...
public class DatabaseService : IDisposable
{
    ...
    public DatabaseService()
    {
        ...
        _flushTimer = new Timer(FlushQueue, null, 500, 500);
    }
    ...
    public async Task<List<ProcessActivityStat>> GetProcessActivityStatsAsync(int topN = 30)
    {
        using var conn = CreateConnection();
        await conn.OpenAsync();
        using var cmd = conn.CreateCommand();
        ...
        var result = new List<ProcessActivityStat>();
        using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
        {
            result.Add(new ProcessActivityStat
            {
